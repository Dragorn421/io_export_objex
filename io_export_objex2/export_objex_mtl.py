...
if TYPE_CHECKING:
    from .export_collect import collect_display_material
...
"""
split write_mtl into write_display and write_collision
but they still write to same file so either open .mtlex in main ObjexExporter and calls the two, or
wrap the two split functions in a new function in here
"""
...
def write_mtl(
    ...
    options,
    copy_set,
    cd_materials,  # type: Dict[Any, collect_display_material.CollectedDisplayMaterial]
    cc_materials,  # type: Dict[Any, collect_collision_mesh.CollectedCollisionMaterial]
):
    ...
    with open(filepath, "w", encoding="utf8", newline="\n") as f:
        ...
        fw('# Display Material Count: {}\n'.format(len(cd_materials)))
        fw('# Collision Material Count: {}\n'.format(len(cc_materials)))
        ...
        def writeTexture(image):
            ...
            if data:
                ...
                if i != 0:
                    log.debug('Texture name {} was already used, using {} instead', base_name, texture_name)
        ...
        scene_settings = {
            ...
            "write_primitive_color": scene.objex_bonus.write_primitive_color,
            "write_environment_color": scene.objex_bonus.write_environment_color,
        }

        for bl_material, cd_material in cd_materials.items():
            log.trace('Writing display material name={!r} name_q={!r} bl_material={!r} cd_material={!r}',
                cd_material.name, cd_material.name_q, bl_material, cd_material)
            util.detect_zztag(log, cd_material.original_name)
            write_display_material(fw, writeTexture, scene_settings, cd_material)

        for bl_material, cc_material in cc_materials.items():
            log.trace('Writing collision material name={!r} name_q={!r} bl_material={!r} cc_material={!r}',
                cc_material.name, cc_material.name_q, bl_material, cc_material)
            util.detect_zztag(log, cc_material.original_name)
            write_collision_material(fw, cc_material)

def write_display_material(
    fw,
    writeTexture,
    scene_settings,
    cd_material,  # type: collect_display_material.CollectedDisplayMaterial
):
    log = getLogger('export_objex_mtl')
    name = cd_material.name
    name_q = cd_material.name_q
    objex_data = cd_material.properties
    ...
    if objex_data.empty or name.startswith('empty.'):
        ...
        return # empty materials do not need anything else written
    ...
    texel_texture_names_q = [None, None]
    for texelIndex, texelAttr in enumerate(('texel0', 'texel1')):
        ...
        if texel is not None:
            ...
            texel_texture_names_q[texelIndex] = writeTexture(image)
    ...
    if cd_material.texel0:
        fw('texel0 {}\n'.format(texel_texture_names_q[0]))
    if cd_material.texel1:
        fw('texel1 {}\n'.format(texel_texture_names_q[1]))
    ...
    if objex_data.rendermode_zmode == 'AUTO':
        otherModeLowerHalfFlags.append('ZMODE_XLU' if cd_material.transparencyType != 'NONE' else 'ZMODE_OPA')
    else:
    ...
    if (objex_data.rendermode_blender_flag_CVG_X_ALPHA == 'YES'
        or (objex_data.rendermode_blender_flag_CVG_X_ALPHA == 'AUTO'
            and cd_material.transparencyType != 'NONE')
    ):
    ...
    if (objex_data.rendermode_forceblending == 'YES'
        or (objex_data.rendermode_forceblending == 'AUTO'
            and cd_material.transparencyType == 'BLEND')
    ):
    ...
    if rm_bl_c0 == 'AUTO':
        if cd_material.transparencyType == 'NONE':
            rm_bl_c0 = 'FOG_SHADE'
        elif cd_material.transparencyType == 'CLIP':
            rm_bl_c0 = 'PASS'
        elif cd_material.transparencyType == 'BLEND':
            rm_bl_c0 = 'XLU'
    if rm_bl_c1 == 'AUTO':
        rm_bl_c1 = 'XLU' if cd_material.transparencyType != 'NONE' else 'OPA'
    presets = {
    ...
    def rgba32(rgba):
        display_device = scene_settings["display_device"]
    ...
    if (
        ...
        and (
            ...
            or (objex_data.write_primitive_color == 'GLOBAL' and scene_settings["write_primitive_color"])
        )
    ):
            ...
            def shiftFromScale(scaleExp):
                ...
                if scaleExp > 0:
                    if scaleExp <= 5:
                        ...
                        log.error('{} scale too big: {}', cd_material.original_name, scaleExp)
                        return 0
                # -10 <= scaleExp <= -1
                # 10 >= n = -scaleExp >= 1
                if scaleExp < 0:
                    if scaleExp >= -10:
                        ...
                        log.error('{} scale too low: {}', cd_material.original_name, scaleExp)
                        return 0
