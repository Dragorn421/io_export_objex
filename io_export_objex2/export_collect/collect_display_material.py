...
class CollectedTexel:
    def __init__(
        ...
        self.image = image  # FIXME document somewhere we aren't collecting image blocks
        self.uScale = uScale
        self.vScale = vScale
        self.uWrap = uWrap
        self.vWrap = vWrap
        self.uMirror = uMirror
...
class CollectedDisplayMaterial:
    def __init__(
        ...
        vScaleMain,
        texgen,
        properties,  # type: properties.ObjexMaterialProperties
        transparencyType,
    ):
        self.original_name = name
        self.name = name
        self.name_q = util.quote(name)
        self.combinerFlags = combinerFlags
        self.primColor = primColor
        self.primLodFrac = primLodFrac
        self.envColor = envColor
        self.shadeSource = shadeSource
        if self.shadeSource not in {None, "NORMALS", "VERTEX_COLORS"}:
            raise util.CollectAbort(
                "Unknown value shadeSource={!r}".format(self.shadeSource)
            )
        self.vertexColorLayer = vertexColorLayer
        self.texel0 = texel0
        self.texel1 = texel1
        self.uvLayer = uvLayer
        self.uScaleMain = uScaleMain
        self.vScaleMain = vScaleMain
        self.texgen = texgen
        if self.texgen not in {None, "TEXGEN", "TEXGEN_LINEAR"}:
            raise util.CollectAbort("Unknown value texgen={!r}".format(self.texgen))
        self.properties = properties
        self.transparencyType = transparencyType
        if self.transparencyType not in {"NONE", "BLEND", "CLIP"}:
            raise util.CollectAbort(
                "Unknown value transparencyType={!r}".format(self.transparencyType)
            )


# FIXME this now has more than node exploring in build(), rename or refactor
class ObjexDisplayMaterialNodeTreeExplorer:
    ...
    def buildFromColorCycle(self, cc):
        ...
        for i in range(4):
            ...
            if (
                socket.bl_idname == "OBJEX_NodeSocket_CombinerOutput"
            ):  # < 2.80 (421FIXME_UPDATE would still be nice if custom color sockets could work in 2.80+)
                flag = socket.flagColorCycle
            else:  # 2.80+
                flag = socket.node["flagColorCycle %s" % socket.identifier]
            if not flag:
                log.error(
                    "No color cycle flag on socket {} of node {}, linked to input {} of {}",
                    socket.name,
                    socket.node.name,
                    "ABCD"[i],
                    cc.name,
            ...
            if flag == "G_CCMUX_COMBINED":
                if prev_color_cycle_node and prev_color_cycle_node != socket.node:
                    log.error(
                        "Different color cycle nodes used for combine {!r} {!r}",
                        prev_color_cycle_node,
                        socket.node,
                    )
                ...
                if socket.node not in self.alphaCycles:
                    log.error(
                        "Color cycle {!r} is combining alpha from alpha cycle node {!r} which was not used in alpha cycles",
                        cc,
                        socket.node,
                    )
                if prev_alpha_cycle_node and prev_alpha_cycle_node != socket.node:
                    log.error(
                        "Different alpha cycle nodes used for combine {!r} {!r}",
                        prev_alpha_cycle_node,
                        socket.node,
                    )
                ...
                if storedFlagSocket and socket != storedFlagSocket:
                    log.error(
                        "Flag {} is used by two different sockets {!r} {!r}",
                        flag,
                        storedFlagSocket,
                        socket,
                    )
    ...
    def buildFromAlphaCycle(self, ac):
        ...
        for i in range(4):
            ...
            if (
                socket.bl_idname == "OBJEX_NodeSocket_CombinerOutput"
            ):  # < 2.80 (421FIXME_UPDATE same as buildFromColorCycle)
                flag = socket.flagAlphaCycle
            else:  # 2.80+
                flag = socket.node["flagAlphaCycle %s" % socket.identifier]
            if not flag:
                log.error(
                    "No alpha cycle flag on socket {} of node {}, linked to input {} of {}",
                    socket.name,
                    socket.node.name,
                    "ABCD"[i],
                    ac.name,
            ...
            if flag == "G_ACMUX_COMBINED":
                if prev_alpha_cycle_node and prev_alpha_cycle_node != socket.node:
                    log.error(
                        "Different cycle nodes used for combine {!r} {!r}",
                        prev_alpha_cycle_node,
                        socket.node,
                    )
                prev_alpha_cycle_node = socket.node
    ...
    def buildCyclesFromOutput(self, output):
        ...
        if output.bl_idname == "ShaderNodeOutput":  # < 2.80
            ...
            lastColorCycleNode = (
                principledBsdfNode.inputs["Base Color"].links[0].from_node
            )
        ...
        if len(self.colorCycles) != len(self.alphaCycles):
            log.error(
                "Not the same amount of color ({:d}) and alpha ({:d}) cycles",
                len(self.colorCycles),
                len(self.alphaCycles),
            )
        flags = []
        for i in range(len(self.colorCycles)):
            ...
            if i == 0 and prev_alpha_cycle_node:
                log.error(
                    "First color cycle node {!r} is combining with alpha cycle {!r} (but the first cycle cannot use combine)",
                    cc,
                    prev_alpha_cycle_node,
                )
            if (
                i > 0
                and prev_alpha_cycle_node
                and prev_alpha_cycle_node != self.alphaCycles[i - 1][0]
            ):
                log.error(
                    "Color cycle {:d} {!r} combines non-previous alpha cycle {!r} instead of previous alpha cycle (based on order) {!r}",
                    i,
                    cc,
                    self.alphaCycles[i - 1][0],
                    prev_alpha_cycle_node,
                )
            flags += colorFlags
            flags += alphaFlags
    ...
    def build(self):
        ...
        if "shade" in self.data:
            if self.data["shade"]["type"] == "vertex_colors":
                shadeSource = "VERTEX_COLORS"
                vertexColorLayer = self.data["shade"]["vertex_color_layer"]
            elif self.data["shade"]["type"] == "normals":
                shadeSource = "NORMALS"
                vertexColorLayer = None
            else:
                # if shadeSource is an invalid value, CollectedDisplayMaterial will raise
                shadeSource = self.data["shade"]["type"]
                vertexColorLayer = None

        texels = [None, None]
        for texelIndex, texelKey in enumerate(("texel0", "texel1")):
            texelData = self.data.get(texelKey)
            if texelData is not None:
                texels[texelIndex] = CollectedTexel(
                    texelData["image"],
                    texelData["uv_scale_u"],
                    texelData["uv_scale_v"],
                    texelData["uv_wrap_u"],
                    texelData["uv_wrap_v"],
                    texelData["uv_mirror_u"],
                    texelData["uv_mirror_v"],
                )
        ...
        if uvMainData is not None:
            uvLayer = uvMainData["uv_layer"]
            uScaleMain = uvMainData["uv_scale_u_main"]
            vScaleMain = uvMainData["uv_scale_v_main"]
            if uvMainData["texgen"]:
                if uvMainData["texgen_linear"]:
                    texgen = "TEXGEN_LINEAR"
                else:
                    texgen = "TEXGEN"

        if hasattr(self.material, "use_transparency"):  # < 2.80
            transparencyType = "BLEND" if self.material.use_transparency else "NONE"
        else:  # 2.80+
            if self.material.blend_method == "OPAQUE":
                transparencyType = "NONE"
            elif self.material.blend_method in {"BLEND", "HASHED"}:
                transparencyType = "BLEND"
            elif self.material.blend_method == "CLIP":
                transparencyType = "CLIP"
            else:
                log.warning(
                    "Material {} uses unknown Blend Mode blend_method={!r}, assuming no transparency",
                    self.material.name,
                    self.material.blend_method,
                )
                transparencyType = "NONE"

        if hasattr(self.material, "use_backface_culling"):  # 2.80+
            if (
                self.material.use_backface_culling
                != self.material.objex_bonus.backface_culling
            ):
                log.warning(
                    "Material {} has backface culling {} in objex properties (used for exporting) "
                    "but {} in the Blender material settings (used for viewport rendering)",
                    self.material.name,
                    "ON" if self.material.objex_bonus.backface_culling else "OFF",
                    "ON" if self.material.use_backface_culling else "OFF",
                )

        return CollectedDisplayMaterial(
            self.material.name,
            self.combinerFlags,
            self.data.get("primitive"),  # primColor
            self.data.get("primitiveLodFrac"),
            self.data.get("environment"),  # envColor
            shadeSource,
            vertexColorLayer,
            texels[0],
            texels[1],
    ...
    def buildCombinerInputs(self):
        ...
        inputs = {
            ...
            "texel0A": (
                ...
                self.buildTexelDataFromAlphaSocket,
            ),
            ...
            "shadeA": (
                ...
                self.buildShadingDataFromAlphaSocket,
            ),
        }
        self.data = {}
        ...
        for k, (flags, socketReader) in inputs.items():
            sockets = set(
                self.flagSockets[flag] for flag in flags if flag in self.flagSockets
            )
            if len(sockets) > 1:
                log.error(
                    "Different sockets {!r} are used by different flags {!r} but these flags should refer to the same data {}",
                    sockets,
                    flags,
                    k,
                )
            if sockets:
                ...
                log.debug(
                    "getting {} from {!r} (socket node: {!r})", k, socket, socket.node
                )
            ...
            if "texel1" in mergedData:  # both texel0 and texel1
                ...
                for k in mergeTexel:
                    if mergedData["texel0"][k] != mergedData["texel1"][k]:
                        log.error(
                            "Could not merge texel0 {!r} and texel1 {!r} main uv transform data into {} in uv_main",
                            mergedData["texel0"][k],
                            mergedData["texel1"][k],
                            k,
                        )
    ...
    def buildColorInputRGB(self, k, socket):
        ...
        if socket.links:
            ...
            if socket.node.bl_idname != "ShaderNodeRGB":
                log.error(
                    "(data key: {}) socket {!r} is not linked to a ShaderNodeRGB node, instead it is linked to {!r} ({})",
                    k,
                    socket,
                    socket.node,
            ...
            log.error(
                "(data key: {}) socket {!r} is not linked (it should, to a ShaderNodeRGB node)",
                k,
                socket,
            )
        if socket.default_value[3] != 1:
            log.warning(
                "(data key: {}) RGB node {} {!r} has its alpha set to {} instead of the default 1\n"
    ...
    def buildColorInputA(self, k, socket):
        ...
        if socket.links:
            log.error(
                "(data key: {}) alpha socket {!r} should not be linked", k, socket
            )
        self.data[k] = socket.default_value
    ...
    def buildSingleValue(self, k, socket):
        ...
        if socket.links:
            log.error(
                "(data key: {}) value socket {!r} should not be linked", k, socket
            )
        self.data[k] = socket.default_value
    ...
    def buildTexelDataFromTextureNode(self, textureNode):
        ...
        if textureNode.bl_idname == "ShaderNodeTexture":  # < 2.80
            if textureNode.texture:
                if textureNode.texture.type != "IMAGE":
                    raise util.ObjexExportAbort(
                        ...
                        "(only image textures can be exported)".format(
                            self.tree.name, textureNode.texture.type
                        )
                    )
            ...
            log.error(
                "Expected a texture node but this is {} {}",
                textureNode.bl_idname,
                textureNode,
            )
        if not textureNode.inputs[0].links:
            raise util.ObjexExportAbort(
                "First input of texture node {} {} has no links".format(
                    textureNode, textureNode.inputs[0]
                )
            )
        ...
        return {
            ...
            "uv_layer": uvSourceNode.uv_layer
            if uvSourceNode.bl_idname == "ShaderNodeGeometry"
            else False,  # < 2.80
            "uv_map": uvSourceNode.uv_map
            if uvSourceNode.bl_idname == "ShaderNodeUVMap"
            else False,  # 2.80+
